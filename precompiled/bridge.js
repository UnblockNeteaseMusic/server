...
var getBuiltIn$d = function (namespace, method) {
  return arguments.length < 2 ? aFunction$h(global$h[namespace]) : global$h[namespace] && global$h[namespace][method];
};
...
var aFunction$f = aFunction$g; // optional / simple context binding
...
var speciesConstructor$7 = function (O, defaultConstructor) {
  ...
  var S;
  return C === undefined || (S = anObject$h(C)[SPECIES$1]) == undefined ? defaultConstructor : aFunction$e(S);
};
...
if (!set || !clear) {
  ...
  if (IS_NODE$2) {
    ...
    defer = bind$6(port.postMessage, port, 1); // Browsers with postMessage, skip WebWorkers
...
var PromiseCapability = function (C) {
  ...
  this.resolve = aFunction$d(resolve);
  this.reject = aFunction$d(reject);
...
setSpecies(PROMISE);
PromiseWrapper = getBuiltIn$7(PROMISE); // statics
...
var cache$a = (job, parameter, live = 30 * 60 * 1000) => {
...
var aFunction$b = aFunction$g; // https://github.com/tc39/collection-methods
...
var aFunction$a = aFunction$g; // https://github.com/tc39/collection-methods

var collectionDeleteAll$1 = function () {
  var collection = anObject$e(this);
  var remover = aFunction$a(collection['delete']);
  var allDeleted = true;
  var wasDeleted;
...
var iterate$d = iterate$f; // `Set.prototype.difference` method
...
var iterate$c = iterate$f; // `Set.prototype.every` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$b = iterate$f; // `Set.prototype.filter` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$a = iterate$f; // `Set.prototype.find` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$9 = iterate$f; // `Set.prototype.intersection` method
// https://github.com/tc39/proposal-set-methods
...
var iterate$8 = iterate$f; // `Set.prototype.isDisjointFrom` method
...
var iterate$7 = iterate$f; // `Set.prototype.isSubsetOf` method
...
var iterate$6 = iterate$f; // `Set.prototype.isSupersetOf` method
...
var iterate$5 = iterate$f; // `Set.prototype.join` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$4 = iterate$f; // `Set.prototype.map` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$3 = iterate$f; // `Set.prototype.reduce` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$2 = iterate$f; // `Set.prototype.some` method
// https://github.com/tc39/proposal-collection-methods
...
var iterate$1 = iterate$f; // `Set.prototype.symmetricDifference` method
// https://github.com/tc39/proposal-set-methods
...
var iterate = iterate$f; // `Set.prototype.union` method
// https://github.com/tc39/proposal-set-methods
...
const check$8 = info => cache$9(search$7, info).then(track$8);
...
const check$7 = info => cache$8(search$6, info).then(track$7);
...
const check$6 = info => cache$7(search$5, info).then(track$6);
...
const check$5 = info => cache$6(search$4, info).then(track$5);
...
const check$4 = info => cache$5(search$3, info).then(track$4);
...
const track$3 = id => {
  ...
  return request$3('POST', url, json_header, json_body, proxy$1).then(response => response.body()).then(body => JSON.parse(body).streamingData).then(streamingData => {
    ...
    return stream.url || (target.sp.includes('sig') ? cache$4(signature, undefined, 24 * 60 * 60 * 1000).then(sign => target.url + '&sig=' + sign(target.s)) : target.url);
  });
};

const check$3 = info => cache$4(key$1 ? apiSearch$1 : search$2, info).then(track$3);
...
const request$2 = request_1; // const proxy = require('url').parse('http://127.0.0.1:1080')
...
const check$2 = info => cache$3(key ? apiSearch : search$1, info).then(track$2);
...
const check$1 = info => cache$2(search, info).then(track$1);
...
const check = info => cache$1(track, info);
...
const distribute = (url, router) => Promise.resolve().then(() => {
  ...
  if (miss || typeof pointer != 'function') return Promise.reject(); // return pointer.call(null, argument)
